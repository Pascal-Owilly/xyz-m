...
>
  {status.status}
    ...
    <td >
      ...
      <button className="btn btn-warning btn-sm" onClick={() => handleUpdateStatus(status.id, 'dispatched')}>
        Dispatched
      </button> &nbsp;&nbsp;
      <button className="btn btn-success btn-sm" onClick={() => handleUpdateStatus(status.id, 'shipped')}>
        Shipped
      </button>
      </div>
    </td>
...
const handleButtonClick = (section) => {
  ...
  return (
    ...
    <div className='main-container container-fluid' style={{ minHeight: '85vh' }}>
      ...
      <Navbar.Brand>
      <h6 className='text-light'>Dispatch & Shipping Stage</h6>
      </Navbar.Brand>

    </Navbar>

    <section className="" >
    <p className='mt-4'>Here, you'll be responsible for updating logistics statuses, specifically transitioning them from the "Dispatched" stage to "Shipped." Your attention to detail and timely updates in this area are crucial for maintaining accurate records and ensuring transparency in our shipping processes. </p>
